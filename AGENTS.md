# AGENTS.md — Talon Pilot 项目契约(codex / 通用 agent 入口)

> 本文件与 [CLAUDE.md](./CLAUDE.md) **内容一致**,只是文件名不同 ——
> claude-code 默认读 `CLAUDE.md`,codex 默认读 `AGENTS.md`,gemini-cli 读
> `GEMINI.md`。我们把同一份契约放进 `CLAUDE.md` + `AGENTS.md` 两份,
> 让所有主流 agent CLI 启动时都能自动加载到。
>
> **维护规则**:改一份必须同步另一份。本文件正文跟 `CLAUDE.md` 完全一致。
...
- `.talon-pilot/superpowers/` 提供方法论 skill(brainstorming / TDD /
  executing-plans 等),要 Read 它们之前 **先看 `superpowers/AGENTS.md`**。
...
chair 必须 **Edit `CLAUDE.md` + `AGENTS.md` 的「§3 项目特定约定」节,
加一条,然后 commit**。让下一轮 turn 的 LLM 自动看到。**禁止**只在群里
口头同意就过。
...
└── .talon-pilot/         # 平台元信息 + 方法论 superpowers
    ...
    ├── openspec/
    └── superpowers/
        ├── AGENTS.md     # 方法论入口(进 superpowers 时再看)
        └── skills/
```
...
- 拿不准 brainstorm / TDD / 写 plan 流程 → Read
  `.talon-pilot/superpowers/AGENTS.md` 找对应 skill。
